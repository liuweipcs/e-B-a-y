...
class EbaysourcedeleteController extends UebController
{
    public function actionReplaceresource()
    {
        ...
        if(isset($_GET['line']))
        {
            $idRange = ['start'=>270001,'end'=>285600];  # ID范围,ID在根目录的表中
            $startTime = time();
            $line = $_GET['line'];
...
//            $data = (new Ebayonlinelisting())->getDbConnection()->createCommand("select * from ueb_ebay_variation_temp where itemid=263268637236")->queryAll();
                ...
                switch($response->Ack->__toString()) {
                    ...
                    case 'Success':
                        ...
                        if (isset($response->Item->Variations)) {
                            ...
                            {
                                $pictureSet = [
                                    'VariationSpecificValue'=>$variationSpecificPictureSet->VariationSpecificValue->__toString()
                                ];
...
//                        echo htmlspecialchars($egdApi->requestXmlBody());
