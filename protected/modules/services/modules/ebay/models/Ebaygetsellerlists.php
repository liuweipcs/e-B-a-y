...
//        var_dump($response);exit();
    ...
    public function updatelist($itemvalue, $storeName,$xmlDataTime) {
        ...
        if(empty($listingObj)) {
           ...
           $id = $listingObj->id;
           if($listingObj->status == '1') { //listing 已修改等待更新，不用做同步数据  1 有修改等待更新
        ...
        if(is_object($itemvalue->Location)) {
            $location = (array)$itemvalue->Location;
            
            $listingObj->location = $location['0'];
        } else {
        ...
        $listingObj->postcode = is_string($itemvalue->PostalCode)?$itemvalue->PostalCode:json_encode($itemvalue->PostalCode);
        ...
        //保存流量销售统计
        UebModel::model('Ebayonlistingbrowsestatis')->UpdateBrowseStatis($itemvalue->ItemID,$itemvalue->HitCount,$itemvalue->WatchCount,$itemvalue->SellingStatus->QuantitySold);
    ...
    public function updatevariations($itemid, $variations) {
        ...
        foreach($variation as $variationvalue) {
            ...
            if(!is_array($nameValueList)) {
                $tempNameValueList = $nameValueList;
                $nameValueList = array();
                $nameValueList['0'] = $tempNameValueList;
            }
...
//        $ShipToLocations = $shipping->ShipToLocations;
        //判断是国内还是国际  运输方式是否存在
//        echo "<pre>";var_dump($ShippingServiceOptions);exit();
        if(!is_array($ShippingServiceOptions)) {
            $tempShipServiceObj = $ShippingServiceOptions;
            $ShippingServiceOptions = array();
            $ShippingServiceOptions['0'] = $tempShipServiceObj;
        }
...
//          $listingObj->ship_to_locations = '';
        ...
        if(!empty($InternationalShippingServiceOption)) {
            if(!is_array($InternationalShippingServiceOption)) {
                $tempInternationObj = $InternationalShippingServiceOption;
                $InternationalShippingServiceOption = array();
                $InternationalShippingServiceOption['0'] = $tempInternationObj;
            }
            ...
            foreach($InternationalShippingServiceOption as $internashippingvalue) {
                ...
                $listingObj = UebModel::model('Ebayonlinelistingshipping')
                    ->find('item_id=:itemid AND shipping_service=:shippingservice AND ship_to_location=:intershippinglocation and shipping_status=:s',array(':itemid'=>$itemid, ':shippingservice'=>$internashippingvalue->ShippingService, ':intershippinglocation'=>$intershippinglocation,':s'=>2));
                if(empty($listingObj)) {
                    $listingObj = new Ebayonlinelistingshipping();
                }
...
//        $listingObj->variation_id = '';
        ...
        if(!is_array($PictureURL)) {
            $tempPicUrl = $PictureURL;
            $PictureURL = array();
            $PictureURL['0'] = $tempPicUrl;
        }
    ...
    public function updateattributes($itmeid, $data) {
        $tempArr = array();
        if(!empty($data)) {
            if(!is_array($data->NameValueList)) {
                $tempNameValue = $data->NameValueList;
                $data->NameValueList = array();
                $data->NameValueList['0'] = $tempNameValue;
            }
