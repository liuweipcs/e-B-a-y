    ...
    'columns' => array(
       array(
        ...
        array(
            ...
            'value'=>function($data){
                if($data->operator){
                    $userName = UebModel::model('User')->getUserNameAndFullNameById($data->charge);
                    return $userName[$data->charge];
                }
                return '';
            },
            'htmlOptions' 	=> array('style' => 'width:80px',),
        ...
        array(
            ...
            'value'=>function($data){
                if($data->operator){
                    $userName = UebModel::model('User')->getUserNameAndFullNameById($data->group_leader);
                    return $userName[$data->group_leader];
                }
                return '';
            },
            'htmlOptions' 	=> array('style' => 'width:80px',),
        ...
        array(
    		...
    		'buttons' => array(
				'groupupdate' => array(
					...
					'url' => 'Yii::app()->createUrl("/systems/wish/groupupdate", array("id" => $data->id))',
					'title' => '编辑',
					'options' => array('target' => 'dialog','width'=>500,'height'=>200,'class'=>'btnEdit'),
				),
				'delete' => array(
					'url'       => 'Yii::app()->createUrl("/systems/wish/groupdelete", array("id" => $data->id))',
					'label'     => '删除',
					'options'   => array(
						'target'    => 'dialog',
						'class'     => 'btnDel',
