    ...
    <div class="pageFormContent" layoutH="56">
        <div class="pd5" style="height:150px;">
            <div class="row">
                ...
                <?php echo $form->dropDownList($model,'user_id',array_column(UebModel::model('User')->getAllMemberByDepId(15),'user_full_name','id'),$userOption); ?>
                <?php echo $form->error($model,'user_id');?>
            </div>
            <div class="row">
                <?php echo $form->labelEx($model, 'ebay_account_id');?>
                <div style="border: 1px solid #9AA4BA;float:left;margin-top: 2px;-webkit-box-shadow: inset 0 0 5px #9AA4BA;width:200px;overflow: auto;max-height: 295px" class="ebay_account_checkbox_area">
                    ...
                    if($this->action->id == 'edit')
                    {
                        $groupId = UebModel::model('EbayGroupMapUser')->find('user_id=:user_id and is_delete=0',array(':user_id'=>$model->user_id))->group_id;
                        $accountIds = array_column(VHelper::selectAsArray('EbayAccountMapGroup','ebay_account_id',"group_id='$groupId'",true),'ebay_account_id');
                        $data = array_column(VHelper::selectAsArray('Ebay','id,user_name','id in ("'.implode('","',$accountIds).'")'),'user_name','id');
                        echo $form->checkBoxList($model,'ebay_account_id',$data);
                    }
                    ?>
                    </div>
                </div>
                <?php echo $form->error($model, 'ebay_account_id'); ?>
            </div>
        </div>
    </div>
    <div class="formBar">
