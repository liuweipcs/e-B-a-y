 ...
 */
Class AmazonClothing extends AmazonUpload implements IAmazonUpload
{
    /**
     * @inheridoc
     *
    ...
    protected function removeEmptyItem2(array $data,$pcate,$ccate)
    {
        ...
        foreach ($data['ProductData'][$pcate][$ccate] as $key => $value) {
            if ($this->isEmpty($value)) {
                unset($data['ProductData'][$pcate][$ccate][$key]);
            }
        }
            ...
            $data['ProductData'] = array(
                'Clothing' => array(
                    'ClassificationData' =>array(
                        // 'Size' => $this->product->sku,
                        // 'Color' => $this->product->sku,
                        'ClothingType'=>'Shirt',
                        'Department'=>'mens',
                    )
                ),
            ...
            $data['ProductData'] = array(
                'Clothing' => array(
                    ...
                    'ClassificationData' =>array(
                        ...
                        'ClothingType'=>'Shirt',
                        'Department'=>'mens',
                        'ColorMap' =>$this->product->sku,
                        'SizeMap' =>$this->product->sku
                    )
                ),
            );
            ...
            foreach ($this->product->sonskues as $sonprd) {
                ...
                $child['ProductData'] = array(
                    'Clothing' => array(
                        ...
                        'ClassificationData' =>array(
                            // 'Size'=>$size?$size:$sonprd->seller_sku,
                            // 'Color'=>$color?$color:$sonprd->seller_sku,
                            'Size'=>$size?$size:'size',
                            'Color'=>$color?$color:'color',
                            'ClothingType'=>'Shirt',
                            'Department'=>'mens',
                            // 'ColorMap' =>$color?$color:$sonprd->seller_sku,
                            // 'SizeMap' =>$size?$size:$sonprd->seller_sku
                ...
                $xmls[$sonprd->sku] = $this->arr2xml->buildXML($this->removeEmptyItem2($child,'Clothing','ClassificationData'), 'Product');
            }
        }
