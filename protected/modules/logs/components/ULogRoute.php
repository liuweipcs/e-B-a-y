<?php
/**
 * ULogRoute class file, base api log route class
 *
 * @author Bob <Foxzeng>
 * 
 */
abstract class ULogRoute extends CComponent
{
	/**
    ...
    /**
     *
     * @var string log type, ebay, amazon, aliexpress etc.;
     */
    public $types = '';
     ...
     * @param string $requestUrl message request url
	 * @param integer $time timestamp
	 * @return string formatted message
	 */
	protected function formatLogMessage($tag, $message, $level, $key, $requestUrl, $time)
	{
		$logMessage = @date('Y-m-d H:i:s',$time)." [$tag] [$level]";
        if (! empty($key) ) {
           $logMessage .= " [$key] " ;
        }
        if (!empty($requestUrl)) {
	...
	{             
		$logs=$logger->getLogs($this->levels, $this->types);      
