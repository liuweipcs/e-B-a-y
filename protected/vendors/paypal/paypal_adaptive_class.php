...
class PayPal_Adaptive extends PayPal
{
	...
	function ConvertCurrency($DataArray)
	{
		...
		$DOM -> loadXML($XMLResponse);
		$Ack = $DOM -> getElementsByTagName('ack') -> length > 0 ? $DOM -> getElementsByTagName('ack') -> item(0) -> nodeValue : '';
		$message = $DOM -> getElementsByTagName('message') -> length > 0 ? $DOM -> getElementsByTagName('message') -> item(0) -> nodeValue : '';
		$rate = $DOM -> getElementsByTagName('currency') -> length > 0 ? $DOM ->getElementsByTagName('currency')->item(0) : array();
		if($rate){
			$rate = $rate->getElementsByTagName('amount') -> length > 0 ? $rate->getElementsByTagName('amount') -> item(0) -> nodeValue : '';
		}else{
			$rate = '';
		}
