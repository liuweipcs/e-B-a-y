 ...
 * @since 1.1
 */
class UebListGridView extends CBaseListView
{
...
// 		echo '</div>';
	...
	public function renderItems()
	{
		...
		{			
            ...
            echo CHtml::openTag('div', $this->tableOptions);
    ...
    public function renderPagerhidden() {
        ...
        $html = '';
        $html .= CHtml::hiddenField('filter', @$_REQUEST['filter']);
	...
	public function renderTableRow($row)
	{
        ...
        foreach($this->columns as $colNum => $column) {
            ...
            $cellConntent = ob_get_clean();

            if (isset($column->name)) {
            	$dataCells[$column->name] = $cellConntent;
            }
        }
        echo preg_replace_callback("/{(\w+)}/",function($matches) use ($dataCells) {
        	$key = $matches[1];
        	if (isset($dataCells[$key])) return $dataCells[$key];
	...
	public function renderMouserRight(){    
		...
		if (is_string($this->toolBar)){
            ...
            foreach ($this->toolBar as $item){
            	...
            	$div.="<li>".CHtml::link($item['text'],$urls,$item['htmlOptions'])."</li>";       
    	...
    	if(!$this->searchNoDisplay){
	    	...
	    	$tpl .= '<script>
	    		...
	    		$("#reset", $p).click(function(){
	    			...
	    			var filter = $(this).closest("#pagerForm").find("input[name=filter]");
	    			if (filter.size() >0) {filter.val("");}
