...
class CUrlManager extends CApplicationComponent
{
	...
	public function createUrl($route,$params=array(),$ampersand='&')
	{
		...
		foreach($this->_rules as $i=>$rule)
		{
			...
			{
				if($rule->hasHostInfo)
					return $url==='' ? '/'.$anchor : $url.$unique.$anchor;
				else
					return $this->getBaseUrl().'/'.$url.$unique.$anchor;
			}
		}
		return $this->createUrlDefault($route,$params,$ampersand).$unique.$anchor;
	}
