...
class CHtml
{
	...
	public static function link($text,$url='#',$htmlOptions=array())
	{
        ...
        if ( isset($htmlOptions['forward'])) {
            $htmlOptions['onclick'] = '$.alertLink(this)';
        }
	...
	{   if (! empty($htmlOptions['validation'])) {
            self::validateOption($name, $htmlOptions);
            self::clientChange('blur', $htmlOptions);
        } else {
...
EOD;
	...
	//
	public static function radioButtonListD($name,$select,$data,$htmlOptions=array())
	{
		$template=isset($htmlOptions['template'])?$htmlOptions['template']:"<div class='radio_style'>{input} {label}";
		$separator=isset($htmlOptions['separator'])?$htmlOptions['separator']:"</div>";
		$container=isset($htmlOptions['container'])?$htmlOptions['container']:'span';
		unset($htmlOptions['template'],$htmlOptions['separator'],$htmlOptions['container']);
	...
	protected static function clientChange($event,&$htmlOptions)
	{
		...
		if(isset($htmlOptions['confirm']))
		{
			...
			if($handler!=='')
                ...
                if (! isset($_SESSION['registerScript'][$id])) {                   
                    if ($live) 
                   ...
                   $_SESSION['registerScript'][$id] = true;
                }
     ...
     */
    protected static function validateOption($name, &$htmlOptions = array()) {
        $pathInfo = Yii::app()->request->getPathInfo();
        $pathArr = explode("/", $pathInfo);        
        $htmlOptions['live'] = true;
        $htmlOptions['id'] = str_replace( array('[',']'), array( '_', ''), $name);
        if (!isset($htmlOptions['errorId']))
            $htmlOptions['errorId'] = $htmlOptions['validation'] . '_em';
        $htmlOptions['ajax'] = array(
            'url'       => "/{$pathArr[0]}/{$pathArr[1]}/validate/fieldName/{$name}/model/{$htmlOptions['validation']}",
            'type'      => 'post',          
            'success'   => 'function(html){               
                var $p = $.pdialog.getCurrent() != null ? $.pdialog.getCurrent() : navTab.getCurrentPanel();
                var errorObj = $("#'.$htmlOptions['errorId'].'", $p);
                    errorObj.html(html);
                    if ( html == "" || html == null) { 
                        errorObj.hide();
                    } else { 
