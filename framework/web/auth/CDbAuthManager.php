...
class CDbAuthManager extends CAuthManager
{
		...
		if($this->executeBizRule($item->getBizRule(),$params,$item->getData()))
		{
			...
			if( $item->getType()==0 ){
				$parents=$this->db->createCommand()
					->select('parent')
					->from($this->itemChildTable.' AS c')
					->leftJoin($this->itemTable.' AS i', 'i.name = c.parent')
			...
			{
				if($aa = $this->checkAccessRecursive($parent,$userId,$params,$assignments)){
					return true;
				}
			}
		}
	...
	public function addItemChild($itemName,$childName)
	{
		...
		if(count($rows)==2)
		{
			...
			if(!$this->hasItemChild($itemName,$childName)){
