...
abstract class CGridColumn extends CComponent
{
    ...
    public static $getConfigwidthcolor = array();

    
    /**
     * @return array get row theme config
     */
    public static function getConfigwidthcolor() {
    	return array(
    			'red'            => '#EE3B3B',
    			'green'          => '#006400',
    			'yellow'         => '#CD950C',
	 ...
	 */
	public function renderDataCell($row, $colNum)
	{
        ...
        if ( isset($this->type) && $this->type == 'Number' ) {   
                if(isset($options['class']))
					$options['class'].= 'text-align-right';
				else
					$options['class'] = 'text-align-right';              
        }
        
        if ( $colNum == 0 ) {
            if (! empty($this->grid->dataProvider->data[$row]) ) {                          
                if ( empty(self::$rowBackgroundConfig) ) {
                    if ( method_exists($this->grid->dataProvider->model, 'getRowBackgroundConfig') ) {
                        self::$rowBackgroundConfig = $this->grid->dataProvider->model->getRowBackgroundConfig();
                    }
                }
                if (! empty(self::$rowBackgroundConfig) ) {
                    $rowBackgroundConfig = self::$rowBackgroundConfig;
                    $rowColorConfig      = self::getConfigwidthcolor();
                    $data = $this->grid->dataProvider->data[$row];
                    foreach ($rowBackgroundConfig as $key => $val) {
                        if ( isset($val[$data->$key]) ) {
                            $backgroundClass = 'background-color:'.$rowColorConfig[$val[$data->$key]].";";
                           
                            if(isset($options['style'])) 
                            $options['style'] .= " " .$backgroundClass;
                            else 
                            $options['style'] = $backgroundClass;                               
                        }
                    }  
