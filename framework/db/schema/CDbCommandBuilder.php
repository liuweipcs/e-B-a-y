...
class CDbCommandBuilder extends CComponent
{
	...
	public function createFindCommand($table,$criteria,$alias='t')
	{
		...
		$auther = array();
		$meunrow = array();
		$meunrow = $command->queryRow(1);
		if(UebModel::model('user')->isAdmin()){
			return $command;
		}
		if(!empty($meunrow)){
			if(!empty($meunrow['menu_status']) && $meunrow['menu_status']==1){
				$menuid = 'menu_'.$meunrow['id'];
				$flag = false;
				foreach($authass as $author=>$authval){
					if(Yii::app()->authManager->hasItemChild($author,$menuid)){
						$flag = true;
						continue;
					}
				}
				if($flag){
					...
					$sql2=($criteria->distinct ? 'SELECT DISTINCT':'SELECT')." {$select} FROM {$table->rawName} $alias";
					$sql2=$this->applyJoin($sql2,$criteria->join);
					$sql2=$this->applyCondition($sql2,"menu_url = '/xxxxxxx/xxx/xxxx'");
					$sql2=$this->applyGroup($sql2,$criteria->group);
					$sql2=$this->applyHaving($sql2,$criteria->having);
					$sql2=$this->applyOrder($sql2,$criteria->order);
					$sql2=$this->applyLimit($sql2,$criteria->limit,$criteria->offset);
					$command=$this->_connection->createCommand($sql2);
					$this->bindValues($command,$criteria->params);
