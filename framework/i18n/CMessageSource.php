...
abstract class CMessageSource extends CApplicationComponent
{
	...
	protected function translateMessage($category,$message,$language)
	{
		...
		/**
		 * add by ethan start 2014.09.02 判断是否二维数据
		 * @desc update. 2014.09.25,修复$this->_messages[$key]重新赋值后变成一维数组导致后面无法正确获取$contronlName
		 * 
		 */
		if(isset($this->_messages[$key]) && !empty($this->_messages[$key]) ){
			$split = '[arr#%%#arr]';
			if(strpos($message,$split) !==false){
				$contronlArr = explode($split,$message);
				$contronlName = $contronlArr[0];
				if(isset($this->_messages[$key][$contronlName][$contronlArr[1]]))
					return $this->_messages[$key][$contronlName][$contronlArr[1]];
				else return $contronlArr[1];
			}
		}
		/** added by ethan end */
